...
for i = 1:m
  ...
  %
  % THIS IS EXACTLY THE SAME AS learningCurve.m but in this case we are randomly
  % selecting the items in the training set to validate the CV set
  % it's exercise 3.5 from Programming Exercise 5 (ML_assignment_4.pdf)


  for j = 1:50
    random_indices = randperm(i);
    

    theta_train = trainLinearReg(X(random_indices, :), y(random_indices), ...
                                 lambda);
    [J_train, grad_train] = linearRegCostFunction(X(random_indices, :), ...
                                                  y(random_indices), ...
                                                  theta_train, 0);
    error_train(i, j) = J_train;

    [J_cv, grad_cv] = linearRegCostFunction(Xval(random_indices, :), ...
                                            yval(random_indices), ...
                                            theta_train, 0);
    error_val(i, j) = J_cv;
  end
